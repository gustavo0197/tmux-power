...
show_cpu_stat="$(tmux_get @tmux_power_show_cpu_stat false)"
show_mem_stat="$(tmux_get @tmux_power_show_mem_stat false)"
show_battery_stat="$(tmux_get @tmux_power_show_battery_stat false)"
...
if "$show_network_speed"; then
    LS="$LS#[fg=$G06,bg=$G05]$right_arrow_icon#[fg=$TC,bg=$G05] #{download_speed}$download_speed_icon #{upload_speed}$upload_speed_icon #[fg=$G05,bg=$BG]$right_arrow_icon"
else
...
RS="#[fg=$G06,bg=$G07]$left_arrow_icon#[fg=$G06,bg=$G06] #[fg=$TC,bg=$G06]$time_icon %T #[fg=$TC,bg=$G06]$left_arrow_icon#[fg=$G04,bg=$TC] $date_icon %F "
...
if "$show_cpu_stat"; then
    RS="#[fg=$TC,bg=$G07] #[fg=$TC,bg=$G07]#{sysstat_cpu}#[fg=$G07,bg=$G07]$left_arrow_icon$RS"
fi

if "$show_mem_stat"; then
    RS="#[fg=$G07,bg=$BG]$left_arrow_icon#[fg=$TC,bg=$G07] #[fg=$TC,bg=$G07]#{sysstat_mem}#[fg=$TC,bg=$G07] $RS"
fi

if "$show_battery_stat"; then
    RS="#[fg=$TC,bg=$G606]#{battery_status_fg} #{battery_icon} #{battery_percentage} $RS"
fi
